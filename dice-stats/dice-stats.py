...
__lastchange__  = "2025-06-16"
__commit_text__ = "v010 + fix ruff errs :dice-stats.py"
__status__      = "uv run python dice-stats.py # on macOS Sequoia 15.3.1 with Python 3.13.3"
    # All checks passed: ruff check dice-stats.py # based on pyproject.toml file
...
try:
    ...
    print("  uv pip install numpy matplotlib scipy statsd")
    print("  uv run --active python dice-stats.py")
    exit(9)
"""
...
def is_number(s:str) -> bool:
    """Return True if the input string contains all numbers."""
    try:
        float(s)
        return True
    except ValueError:
...
def calculate_histogram_metrics(data, bins=6):
    """
    Return a dictionary containing calculated metrics for a histogram.
