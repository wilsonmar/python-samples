...
import google.auth.transport.requests  # https://google-auth.readthedocs.io/en/master/reference/google.auth.transport.requests.html
...
pgm_start_perf_counter = time.perf_counter() # the most accurate difference between two times. Used by timeit.
...
print_trace("this_pgm_last_modified_datetime="+str(this_pgm_last_modified_datetime)+" (local time)")
...
def gen_hash_text(gen_hash_method, byte_array_in):
	...
	if show_verbose:
		print_verbose(f'{gen_hash_method} {m.block_size}-bit {m.digest_size}-hexbytes {m.digest_size*2}-characters')
...
class TestFibonacci(unittest.TestCase):
	def test_gen_fibonacci(self):
		...
		if gen_fibonacci:
			...
			print_info(f'fibonacci_recursive: {n} => {result} in {timedelta(seconds=recursive_time_duration)} seconds ')
			...
			if show_info == True:
				print_trace(f'fibonacci_memoized: {n} => {result} in {timedelta(seconds=memoized_time_duration)} seconds ({"%.2f" % diff_order}X faster).')
...
def find_ip_geodata(my_ip_address):
	...
	try:
		...
		try:
			...
			print_info(f'{localize_blob("Longitude")}: {ip_base["longitude"]} {localize_blob("Latitude")}: {ip_base["latitude"]} in {ip_base["country_code"]} {ip_base["timezone"]} {ip_base["currency"]} (VPN IP).')
			return ip_base
		except Exception:
...
class TestLookupZipinfo(unittest.TestCase):
	def test_lookup_zipinfo(self):
		...
		if lookup_zipinfo:
			...
			print_trace(f'lookup_zipinfo: zippopotam_url={zippopotam_url}')
			try:
				...
				y = x["places"]
				print_info(f'lookup_zipinfo: {zip_code} = {y[0]["place name"]}, {y[0]["state abbreviation"]} ({y[0]["state"]}), {x["country abbreviation"]} ({x["country"]})')
				print_info(f'{localize_blob("Longitude:")} {y[0]["longitude"]} {localize_blob("Latitude:")} {y[0]["latitude"]}')
...
if use_azure:
	...
	try:
		...
		print_trace(f'Using Azure secret Key Vault \"{azure_keyVaultName}\" in {azure_region} region.')
	except Exception:
...
def gcp_doc_title(scopes_in,document_id_in):
...
if download_imgs:
	...
	if path.exists(img_project_path):
		...
		print_verbose(f'{localize_blob("Directory")} \"{img_directory}\" {localize_blob("created")} {formatted_epoch_datetime}')
	...
	try:
		...
		print_fail(f'{localize_blob("Directory path")} \"{img_file_name}\" {localize_blob("cannot be created")}.')
...
if download_imgs:
	...
	if path.exists(img_directory):
		...
		print_trace(f'{localize_blob("Directory")} \"{img_directory}\" {localize_blob("created")} {formatted_epoch_datetime}')
	...
	if path.exists(img_file_path):
		...
		print_verbose(f'Download of {format_number( os.path.getsize(img_file_path))}-byte {img_file_name} ')
...
if cleanup_img_files:
	...
	if remove_img_file_at_end:
		print_verbose(f'{localize_blob("File")} \"{img_file_path}\" {localize_blob("being removed")} ')
		file_remove(img_file_path)
